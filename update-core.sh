...
echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  OCA — OpenClaw on Android Updater v${OCA_VERSION}${NC}"
echo -e "${BOLD}========================================${NC}"
echo ""
...
# Legacy migration
OLD_DIR="$HOME/.openclaw-pocket-server"
...
if [ "$IS_GLIBC" = false ]; then
    ...
    if command -v opencode &>/dev/null; then
        echo -e "${YELLOW}[SKIP]${NC} OpenCode update check (manual: bun install -g opencode-ai)"
    else
...
update_ai_tool "qwen-code" "@qwen-code/qwen-code" "Qwen Code" && AI_FOUND=true
