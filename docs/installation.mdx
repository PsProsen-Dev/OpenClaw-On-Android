...
Installs selected tools: Termux packages via `pkg`, AI CLIs via `npm install -g`, while **Homebrew (Linuxbrew)** and the **Go toolchain** are installed by default as part of the base OCA Linux-like environment.

### Homebrew (`scripts/install-homebrew.sh`)

OCA runs the official Homebrew installer into `~/.linuxbrew` and appends:
...
OCA installs the Termux `golang` package so `go install ...` workflows are ready before users start selecting OpenClaw skills, helper CLIs, and small utilities.

### Gemini/Qwen fixer (`scripts/termux_cli_manager.py`)

OCA now includes a small manager for Gemini CLI and Qwen Code on Termux. It can:

- ensure `ripgrep` is installed from `pkg`
- install/update the CLIs with `--ignore-scripts`
- recreate stable wrappers that force execution through OCA's glibc Node.js
- verify that wrapper, entrypoint, and `rg` are all present
